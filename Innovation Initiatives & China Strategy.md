## Meeting Summary – Innovation Initiatives & China Strategy

### 1. **Purpose of the Meeting**

- Introductory session to explain **ACWA’s innovation approach** and how individual initiatives fit into the broader strategy.
- Introduction of **Sharzi Mukhri’s role** in leading innovation initiatives.
- Preparation for deeper work on **China-focused innovation activities**.

### 2. **Innovation Management Approach**

- Innovation pipeline is managed by **Technology Readiness Levels (TRL/TL)**:
    - **Low TRL (TL3–TL5)**: Maintain relationships, sign NDAs, monitor progress.
    - **TL6+**: Actively engage (pilot plants, partnerships, investment).
- Goal: **Stay close to high‑potential ideas early** so ACWA can act quickly when they mature.
- Example: a **low‑TRL ammonia technology** from the US is monitored but not actively deployed yet.
- Monitoring includes:
    - Funding status
    - Partners involved
    - Technology progress
- If no potential → disengage; if high potential → move fast.

### 3. **Strategic Geographic Focus**

- **China identified as a critical strategic layer**, alongside Saudi Arabia and Uzbekistan.
- ACWA wants early engagement with **key Chinese players before competitors**.
- Strategy combines:
    - **Technology scouting**
    - **Geographic prioritization**
    - **Relationship-building via associations**

### 4. **China Innovation Ecosystem & Partnerships**

- Key associations and partners:
    - **SNEC** (important innovation platform in China)
    - **NICE (National Innovation Center for Excellence)**
    - Leading universities
- Prior innovation challenges in China generated interest but **idea quality was insufficient**, so programs were paused.
- Future focus:
    - Better-focused initiatives
    - Higher-quality deal flow
    - Stronger alignment with ACWA needs

### 5. **Action Items for China Innovation**

**Two immediate priorities:**

1. **Define innovation initiatives in China**
    - Robotics identified as a top priority.
2. **Organize “Innovation Days – China (Shanghai)”**
    - Signature ACWA innovation event.

### 6. **Database of Chinese Companies & Key People**

- Create a **living database** of:
    - Strategic companies relevant to ACWA
    - Key technical & R&D leaders (CTO, VP R&D, Innovation Heads)
- Initial list includes: Sungrow, Envision, LONGi, Jinko, etc.
- Use:
    - Chinese platforms + BD & procurement teams
    - Annual updates after initial setup
- Purpose: enable **targeted engagement and fast execution of MOUs**.

### 7. **Technology Gaps & Focus Areas**

- **Biggest internal gap**: Industrial AI.
- Strength areas already strong:
    - Solar PV
    - Batteries (BYD, CATL, sodium-ion)
    - Hydrogen
- China is critical for:
    - Robotics
    - Industrial AI
    - Advanced energy storage (flow batteries, sodium batteries)

### 8. **Innovation Days – Key Design Principles**

- Event built around a **people-driven agenda**:
    - Topics defined first
    - Then select best speakers (industry + top universities)
- Mix of:
    - Large industry players
    - Academia
    - Start-ups (via competitions or challenges)
- Robotics should be **visible and experiential**, creating the image:
    
    > _ACWA = future-ready, leading in industrial AI and robotics_
    

### 9. **Strategic Objectives (Reaffirmed)**

1. **Remain global leader in desalination**
2. **Become #1 in hydrogen**
3. **Lead in energy storage**
4. **Build credibility and leadership in industrial AI**

### 10. **Next Steps**

- Thomas to send **Innovation China Strategy document** for review.
- Sharzi to share:
    - Stakeholder framework
    - Key messaging
    - Initial agenda skeleton
    - Execution plan
- Follow-up meetings scheduled to finalize the China innovation roadmap.

If you want, I can:

- Turn this into a **one-page executive brief**
- Extract **explicit action items with owners**
- Summarize **China Innovation Strategy only**